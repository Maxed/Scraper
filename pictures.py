...
img = (".png", ".jpg", ".jpeg", ".gif", ".webm", ".gifv", ".bmp")
...
def check_submission(url, file_name):
    if url.endswith(img):
        f = CURRENT_PATH + "/" + get_file_name(url, file_name)
        if not os.path.isfile(f):
            add_to_queue(url, get_file_name(url, file_name))
...
def get_submissions():
    ...
    for submission in reddit.subreddit(SUBREDDIT).top(limit = LIMIT):
        ...
        if id not in reddit_id_cache: #TODO change this to url so the same url on different subs doesnt get dled
            ...
            print(id + " added to reddit_id_cache")
